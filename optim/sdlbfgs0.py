...
class SdLBFGS0(Optimizer):
    ...
    def __init__(self, params, lr=1, delta=1e-3, lr_decay=True, weight_decay=0, max_iter=1, max_eval=None,
                 tolerance_grad=1e-5, tolerance_change=1e-9, history_size=100,
                 line_search_fn=None):
        if max_eval is None:
            max_eval = max_iter * 5 // 4
        defaults = dict(lr=lr, delta=delta, lr_decay=lr_decay, weight_decay=weight_decay, max_iter=max_iter,
                        max_eval=max_eval,
                        tolerance_grad=tolerance_grad, tolerance_change=tolerance_change,
                        history_size=history_size, line_search_fn=line_search_fn)
        super(SdLBFGS0, self).__init__(params, defaults)
